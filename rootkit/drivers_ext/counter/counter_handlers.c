...
{
	Z_OOPS(Z_SYSCALL_OBJ(dev, K_OBJ_DRIVER_COUNTER));
	return z_impl_counter_us_to_ticks((const struct device *)dev, (uint64_t)us);
}
#include <syscalls/counter_us_to_ticks_mrsh.c>

static inline uint64_t z_vrfy_counter_ticks_to_us(const struct device *dev, uint32_t ticks)
{
	Z_OOPS(Z_SYSCALL_OBJ(dev, K_OBJ_DRIVER_COUNTER));
	return z_impl_counter_ticks_to_us((const struct device *)dev, (uint32_t)ticks);
}
...
{
	Z_OOPS(Z_SYSCALL_DRIVER_RTC(dev, set_value));
	return z_impl_counter_set_value((const struct device *)dev, ticks);
}
#include <syscalls/counter_set_value_mrsh.c>
												   ...
												   const struct counter_alarm_cfg *alarm_cfg)
{
	...
	Z_OOPS(Z_SYSCALL_VERIFY_MSG(cfg_copy.callback == 0, "callbacks may not be set from user mode"));
...
static inline int z_vrfy_counter_cancel_channel_alarm(const struct device *dev, uint8_t chan_id)
{
	...
	return z_vrfy_counter_cancel_channel_alarm((const struct device *)dev, (uint8_t)chan_id);
}
#include <syscalls/counter_cancel_channel_alarm_mrsh.c>

static inline int z_vrfy_counter_set_top_value(const struct device *dev, const struct counter_top_cfg *cfg)
{
	...
	Z_OOPS(Z_SYSCALL_VERIFY_MSG(cfg_copy.callback == 0, "callbacks may not be set from user mode"));
	return z_impl_counter_set_top_value((const struct device *)dev, (const struct counter_top_cfg *)&cfg_copy);
}
...
{
	Z_OOPS(Z_SYSCALL_OBJ(dev, K_OBJ_DRIVER_COUNTER));
	return z_impl_counter_get_guard_period((const struct device *)dev, flags);
}
#include <syscalls/counter_get_guard_period_mrsh.c>

static inline int z_vrfy_counter_set_guard_period(const struct device *dev, uint32_t ticks, uint32_t flags)
{
	Z_OOPS(Z_SYSCALL_OBJ(dev, K_OBJ_DRIVER_COUNTER));
...
static inline int z_vrfy_counter_set_alarm_wk_rpt(const struct device *dev, uint8_t chan_id,
												  const struct counter_alarm_wk_rpt_cfg *alarm_cfg)
{
	struct counter_alarm_wk_rpt_cfg cfg_copy;

	Z_OOPS(Z_SYSCALL_DRIVER_COUNTER(dev, set_alarm));
	Z_OOPS(z_user_from_copy(&cfg_copy, alarm_cfg, sizeof(cfg_copy)));
	Z_OOPS(Z_SYSCALL_VERIFY_MSG(cfg_copy.callback == 0, "callbacks may not be set from user mode"));
	return z_impl_counter_set_alarm_wk_rpt((const struct device *)dev, (uint8_t)chan_id,
										   (const struct counter_alarm_wk_rpt_cfg *)&cfg_copy);
}
#include <syscalls/counter_set_alarm_wk_rpt_mrsh.c>
