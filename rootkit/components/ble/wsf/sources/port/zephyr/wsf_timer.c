...
#define CLK_TICKS_PER_WSF_TICKS             (WSF_MS_PER_TICK * Z_HZ_ticks / 1000)
...
static struct k_timer wsf_timer;
...
static void WsfTimer_handler(struct k_timer *timer)
{
  ARG_UNUSED(timer);
...
void WsfTimerInit(void)
{
  ...
  k_timer_init(&wsf_timer, WsfTimer_handler, NULL);
}
...
{
    int64_t ui32CurrentTime, ui32ElapsedTime;
    ...
    //
    ui32CurrentTime = k_uptime_ticks(); //xTaskGetTickCount();
    ...
    if ( xNextExpiration )
    {
        k_timer_start(&wsf_timer, K_MSEC(xNextExpiration*CLK_TICKS_PER_WSF_TICKS), 
            K_FOREVER);
    }
}
