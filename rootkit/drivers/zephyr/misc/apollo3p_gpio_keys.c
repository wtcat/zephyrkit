...
struct key_device {
    struct key_value buf[QKEY_SIZE];
    ...
    void *context)
{
    ...
    if (notify) {
        ...
        key_dev.notify = notify;
        key_dev.context = context;
...
void iterate_gpio_key(void (*iter)(struct key_value *, void *), 
    void *arg)
{
    __ASSERT_NO_MSG(iter != NULL);
    struct key_value val;
    while (!dequeue_gpio_key(&val)) {
        iter(&val, arg);
    }
}
...
void flush_gpio_keys(void)
{
    k_spinlock_key_t key;
    key = k_spin_lock(&key_dev.spin);
    key_queue_reset(&key_dev);
    k_spin_unlock(&key_dev.spin, key);
}

int dequeue_gpio_key(struct key_value *val)
{   
    struct key_device *dev = &key_dev;
    k_spinlock_key_t key;

    key = k_spin_lock(&key_dev.spin);
    if (!key_queue_empty(dev)) {
        *val = dev->buf[dev->head];
