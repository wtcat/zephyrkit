...
/// version string
#define GH30X_EXAMPLE_VER_STRING            "example code v0.1.5 (For hbd_ctrl lib v0.5.3.0 and later)\r\n"
...
#define SEND_MCU_HRV_MODE_RESULT(rr_arr, cnt, lvl, hb, data, len)       send_mcu_hrv_mode_result_pkg(rr_arr, cnt, lvl, hb, data, len)
#define SEND_MCU_SPO2_UNWEAR_EVT(data, len)                             send_mcu_spo2_mode_unwear_pkg(data, len)
#define SEND_MCU_SPO2_MODE_RESULT(spo2, spo2_lv, hb, hb_lv, rr, rr_lv, rr_c, spo2_r, sta, vd, abnsta, data, len)     \
                                                                        send_mcu_spo2_mode_result_pkg(spo2, spo2_lv, hb, hb_lv, rr, rr_lv, rr_c, spo2_r, sta, vd, abnsta, data, len)
#else
...
#define SEND_MCU_HB_MODE_RESULT(hb, lv, sta, wq, vb, ret, rr, current, data, len)
#define SEND_MCU_HRV_MODE_RESULT(rr_arr, cnt, lvl, hb, data, len)
...
#define GSENSOR_SOFT_FIFO_BUFFER_MAX_LEN    (105)
...
#define MCU_PKG_HRV_ALGO_RESULT_LEN         (11)
#define MCU_PKG_SPO2_WEAR_RESULT_LEN        (18)
#define MCU_PKG_SPO2_ALGO_RESULT_LEN        (18)
...
/* debug log control, if enable debug, example_dbg_log must define in platforms code */
    ...
    // #define   EXAMPLE_DEBUG_LOG_L1(fmt, ...) printk(fmt, ##__VA_ARGS__) 
     #define   EXAMPLE_DEBUG_LOG_L1(fmt, ...)
...
void send_mcu_hrv_mode_result_pkg(uint16_t rr_val_arr[], uint8_t rr_cnt, uint8_t rr_lvl, uint8_t hb_val, GS32 rawdata_dbg[][DBG_MCU_MODE_PKG_LEN], uint8_t rawdata_dbg_len);
...
void send_mcu_spo2_mode_result_pkg(uint8_t spo2_val, uint8_t spo2_lvl_val, uint8_t hb_val, uint8_t hb_lvl_val, uint16_t rr_val[4], uint8_t rr_lvl_val, uint8_t rr_cnt, 
							uint16_t spo2_r_val, uint8_t wearing_state_val, uint8_t valid_lvl_val, uint8_t abnormal_state, GS32 rawdata_dbg[][DBG_MCU_MODE_PKG_LEN], uint16_t rawdata_dbg_len);
