...
LOG_MODULE_REGISTER(fs_nvs, CONFIG_NVS_CUSTOM_LOG_LEVEL);

static K_MEM_SLAB_DEFINE(nvs_buff_pool, 256, 4, 4);

/* basic routines */
/* nvs_al_size returns size aligned to fs->write_block_size */
static inline size_t nvs_al_size(struct nvs_fs *fs, size_t len)
{
	uint16_t write_block_size = fs->flash_parameters->write_block_size;
							...
							size_t len)
{
	...
	int err;
	char *buf = NULL;
	err = k_mem_slab_alloc(&nvs_buff_pool, (void **)&buf, K_NO_WAIT);
	if (err) {
		return -ENOSPC;
	}
#endif
	...
	if (!len) {
		...
		k_mem_slab_free(&nvs_buff_pool, (void **)&buf);
		return 0;
	}
							   ...
							   size_t len)
{
	...
	int err;
	char *cmp;
	err = k_mem_slab_alloc(&nvs_buff_pool, (void **)&cmp, K_NO_WAIT);
	if (err) {
		return -ENOSPC;
	}
#endif
	...
	while (len) {
		...
		if (rc) {
			k_mem_slab_free(&nvs_buff_pool, (void **)&cmp);
			return rc;
		}
		len -= bytes_to_cmp;
		addr += bytes_to_cmp;
...
static int nvs_gc(struct nvs_fs *fs)
{
	...
	do {
		...
		do {
			 ...
			 */
			if ((wlk_ate.id == gc_ate.id) && (!nvs_ate_crc8_check(&wlk_ate))) {
				break;
			}
