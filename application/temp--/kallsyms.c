...
extern const unsigned long _ramfunc_ram_start, _ramfunc_ram_end;
...
int is_ksym_addr(unsigned long addr)
{
	...
	if ((addr >= (unsigned long)&_ramfunc_ram_start &&
	     addr <= (unsigned long)&_ramfunc_ram_end))
		return 1;
...
static int print_symb_init(const struct device *dev)
{
	...
	unsigned int len, off;
 	...
 	if(sd_fmap("ksym.bin", &sym_addr, &len)){
	...
	kallsyms_addresses = &sym_addr[off+2];
	...
	kallsyms_markers = &sym_addr[off+2];
	off += sym_addr[off+1]/4+2;	
