...
ALARM_MANAGER alarm_manager = {
	.alarm_table =
		{
			{0xaa, "11", "03", GX_STRING_ID_TIME_AM, 0x7f, ALARM_ON},
			{0xff, "11", "02", GX_STRING_ID_TIME_AM, 0x7f, ALARM_ON},
			{0xff, "33", "33", GX_STRING_ID_TIME_AM, 0x7f, ALARM_OFF},
			{0xff, "44", "44", GX_STRING_ID_TIME_PM, 0x0f, ALARM_OFF},
			{0xff, "55", "55", GX_STRING_ID_TIME_PM, 0x0f, ALARM_OFF},
			{0xff, "66", "66", GX_STRING_ID_TIME_PM, 0x0f, ALARM_OFF},
			{0xff, "77", "77", GX_STRING_ID_TIME_PM, 0x0f, ALARM_OFF},
			{0xff, "88", "88", GX_STRING_ID_TIME_PM, 0x0f, ALARM_OFF},
			{0xff, "99", "99", GX_STRING_ID_TIME_PM, 0x0f, ALARM_OFF},
			{0xff, "00", "00", GX_STRING_ID_TIME_PM, 0x0f, ALARM_OFF},
		},
...
static uint32_t find_earliest_alarm(struct tm *tm_now, ALARM_INFO *alarm_info, uint8_t *hour, uint8_t *min,
									uint8_t *weekday)
{
...
static void alarm_list_ctl_alarm_update(void)
{
	...
	for (uint8_t i = 0; i < total_cnt; i++) {
		if (alarm_manager.alarm_table[i].alarm_status == ALARM_ON) {
			...
			uint32_t delta_curr = find_earliest_alarm(tm_now, &alarm_manager.alarm_table[i], &hour, &min, &weekday);
			if (delta_curr <= delta) {
				delta = delta_curr;
...
{
	settings_save_one("setting/alarm/alarms", (void *)&alarm_manager, sizeof(ALARM_MANAGER));
	return;
}
