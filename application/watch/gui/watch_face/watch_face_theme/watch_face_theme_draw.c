...
extern int watch_face_color_get(uint32_t index, void *addr);
extern int watch_face_pixelmap_get(uint32_t index, void **addr);

static uint16_t wf_get_rotate_degree(element_type_enum type,
...
void wf_draw_single_img(struct wf_element_style *style, GX_WIDGET *widget)
{
	...
	degree_total = degree_total <= 0 ? degree_total + 360
									 : (degree_total > 360 ? degree_total % 360
														   : degree_total);
	watch_face_pixelmap_get(s_img_para->res_id, (void **)&map);
	if (map == NULL) {
		return;
	}
	if (s_img_para->element_rotate_type == ELEMENT_ROTATE_TYPE_ROTATING) {
		...
		GX_VALUE top =
			s_img_para->y_pos + widget->gx_widget_size.gx_rectangle_top;
		GX_VALUE left =
			s_img_para->x_pos + widget->gx_widget_size.gx_rectangle_left;
...
void wf_draw_multi_img_one(struct wf_element_style *style, GX_WIDGET *widget)
{
	...
	case ELEMENT_TYPE_WEEK_DAY:
		...
		uint8_t curr_week_day = watch_face_get_week(); // week: 0~6
		if (curr_week_day >= 7) {
			curr_week_day = 0;
		}
		if (0 == curr_week_day) {
	...
	case ELEMENT_TYPE_AM_PM:
		...
		id = watch_face_get_am_pm_type() + para->res_id_start;
		break;
	default: // do nothing
		return;
	}
	watch_face_pixelmap_get(id, (void **)&map);
	if (map == NULL) {
		return;
	}
									 ...
									 GX_WIDGET *widget)
{
	...
	for (uint8_t i = 0; i < cnts; i++) {
		if ((i == 0) && (type == MULTI_IMG_TYPE_NXX)) { // temperature or other
			if (value < 0) {
				if (resource_cnts >= 12) { // must 12 images
					watch_face_pixelmap_get(resource_start + 11, (void **)&map);
				} else {
					map = GX_NULL;
				}
			} else {
		...
		left +=
			style->extend_param.ext_disp_para.multi_img_n_para.width_per_unit;
	}
}
...
void wf_draw_arc(struct wf_element_style *style, GX_WIDGET *widget)
{
	...
	if (watch_face_color_get(arc_para->color_id, &brush_color)) {
		brush_color = 0;
	}
	...
	int32_t x_center =
		widget->gx_widget_size.gx_rectangle_left + (arc_para->diameter >> 1);
	int32_t y_center =
		widget->gx_widget_size.gx_rectangle_top + (arc_para->diameter >> 1);
	int32_t angle_total = arc_para->angle_start - arc_para->angle_end;

	angle_total = angle_total <= 0
					  ? angle_total + 360
