...
static int alarm_edit_page_info_update(REMINDERS_EDIT_TYPE type, REMINDER_INFO_T *info);
...
static UINT scroll_page_event_process(GX_WIDGET *widget, GX_EVENT *event_ptr)
{
	...
	case GX_SIGNAL(TIME_EDIT_BUTTON_ID, GX_EVENT_CLICKED):
		...
		app_reminders_page_jump(REMINDERS_PAGE_EDIT, REMINDERS_OP_TIME_SEL);
		break;
	case GX_SIGNAL(RPT_EDIT_BUTTON_ID, GX_EVENT_CLICKED):
		app_reminders_page_jump(REMINDERS_PAGE_EDIT, REMINDERS_OP_RPT_SEL);
		LOG_INF("rpt edit button clicked!");
		break;
	case GX_SIGNAL(VIB_DU_EDIT_BUTTON_ID, GX_EVENT_CLICKED):
		app_reminders_page_jump(REMINDERS_PAGE_EDIT, REMINDERS_OP_VIB_DURATION);
...
static UINT alarm_edit_page_event_process(GX_WIDGET *widget, GX_EVENT *event_ptr)
{
	...
	case GX_EVENT_PEN_UP:
		if (GX_TRUE == pen_down_valid) {
			...
			if (delta_x >= 100) {
				app_reminders_page_jump(REMINDERS_PAGE_EDIT, REMINDERS_OP_CANCEL);
			}
...
void app_reminders_edit_page_init(void)
{
	GX_RECTANGLE size;
...
#endif
	app_reminders_page_reg(REMINDERS_PAGE_EDIT, &curr_page, alarm_edit_page_info_update);
}
