...
static int alarm_time_sel_page_info_update(REMINDERS_EDIT_TYPE type, REMINDER_INFO_T *info);
...
static UINT alarm_time_sel_page_event_process(GX_WIDGET *widget, GX_EVENT *event_ptr)
{
	...
	case GX_EVENT_PEN_UP: {
		...
		if (delta_x >= 100) {
			app_reminders_page_jump(REMINDERS_PAGE_TIME_SEL, REMINDERS_OP_CANCEL);
		}
		break;
	}
	...
	case GX_SIGNAL(NEXT_OR_FINISH_BUTTON_ID, GX_EVENT_CLICKED): {
		...
		app_reminders_page_jump(REMINDERS_PAGE_TIME_SEL, REMINDERS_OP_NEXT_OR_FINISH);
		break;
	}
	default:
		break;
...
void reminders_time_sel_page_init(void)
{
	...
	app_reminders_page_reg(REMINDERS_PAGE_TIME_SEL, &alarm_time_select_page, alarm_time_sel_page_info_update);
}
