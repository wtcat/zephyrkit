...
static Custom_RoundedButton_TypeDef eco_mode_button;
...
void app_setting_eco_mode_page_init(GX_WINDOW *window)
{
	...
	setting_disp_page_children_reg(&curr_page, DISP_SETTING_CHILD_PAGE_ECO_MODE);
	...
	string.gx_string_ptr = "eco mode";
	string.gx_string_length = strlen(string.gx_string_ptr);
	custom_rounded_button_create(&eco_mode_button, ROUND_BUTTON_ID, &curr_page, &childSize,
	...
	custom_checkbox_create(&checkbox, &eco_mode_button.widget, &checkbox_info, &childSize);
