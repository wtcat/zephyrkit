...
static const GX_CHAR *lauguage_ch_setting_element[2] = {"网格", "列表"};
static const GX_CHAR *lauguage_en_setting_element[2] = {"Grid View", "List View"};
...
static UINT widget_event_processing_function(GX_WIDGET *widget, GX_EVENT *event_ptr)
{
	...
	case GX_EVENT_SHOW: {
		uint8_t data;
		setting_service_get(SETTING_CUSTOM_DATA_ID_APP_VIEW_TYPE, &data, 1);
		if (data) {
	...
	case GX_SIGNAL(BUTTON1_ID, GX_EVENT_CLICKED): {
		uint8_t data = 0;
		view_service_event_submit(VIEW_EVENT_TYPE_APP_VIEW_CFG_CHG, &data, 1);
		custom_radiobox_status_change(&curr_page_content.radiobox_type2, 0);
		custom_radiobox_status_change(&curr_page_content.radiobox_type1, 1);
		break;
	}
...
void app_setting_list_style_page_init(GX_WINDOW *window)
{
	...
	setting_main_page_jump_reg(&curr_main_page, SETTING_CHILD_PAGE_APP_VIEW);
