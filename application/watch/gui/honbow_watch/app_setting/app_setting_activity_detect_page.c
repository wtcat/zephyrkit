...
static const GX_CHAR english_tips[] =
	"When the meter detects a movement or ends a movement, it advances to Wake up.The sports "
	"types include walking,running and single walking ";
...
void app_setting_activity_detect_page_init(GX_WINDOW *window)
{
	...
	// disp_setting_jump_reg(&curr_page, DISP_SETTING_CHILD_PAGE_WRIST_RAISE);

	extern void setting_main_page_jump_reg(GX_WIDGET * dst, SETTING_CHILD_PAGE_T id);
	setting_main_page_jump_reg(&curr_page, SETTING_CHILD_PAGE_TRAINING_DETECT);
	...
	string.gx_string_ptr = "Activity Detect";
	string.gx_string_length = strlen(string.gx_string_ptr);
