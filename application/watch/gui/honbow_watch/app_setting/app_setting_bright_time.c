...
static UINT event_processing_function(GX_WIDGET *widget, GX_EVENT *event_ptr)
{
	...
	case GX_SIGNAL(SCROLL_WHEEL_ID, GX_EVENT_LIST_SELECT): {
		uint8_t data = event_ptr->gx_event_payload.gx_event_longdata;
		view_service_event_submit(VIEW_EVENT_TYPE_BRIGHT_TIME_CHG, &data, 1);
		break;
	}
	default:
		break;
...
static GX_CHAR *string_list[] = {"5", "10", "15"};
...
void app_setting_brightTime_page_init(GX_WINDOW *window)
{
	...
	gx_widget_create(&disp_setting_brightness_page, NULL, window, GX_STYLE_ENABLED | GX_STYLE_TRANSPARENT, GX_ID_NONE,
					 &childSize);
	setting_disp_page_children_reg(&disp_setting_brightness_page, DISP_SETTING_CHILD_PAGE_BRIGHT_TIME);
	...
	gx_text_scroll_wheel_create(&brightness_adjust_wheel, GX_NULL, parent, 3,
