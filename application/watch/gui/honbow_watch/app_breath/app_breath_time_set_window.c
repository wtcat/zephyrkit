...
GX_WIDGET set_breath_time_window;
// static uint8_t hr = 0;
static GX_PROMPT title_prompt;
static GX_PROMPT min_prompt;
...
static GX_NUMERIC_SCROLL_WHEEL time_wheel;
...
UINT app_set_breath_time_window_event(GX_WIDGET *window, GX_EVENT *event_ptr)
{
	INT row;
	switch (event_ptr->gx_event_type) {	
	case GX_SIGNAL(2, GX_EVENT_CLICKED):
		gx_scroll_wheel_selected_get(&time_wheel, &row);
		breath_data.breath_time = row + 1;
		// Gui_close_spo2();
		jump_breath_page(&breath_main_page, &set_breath_time_window, SCREEN_SLIDE_RIGHT);
	break;
...
static VOID set_breath_time_child_widget_create(GX_WIDGET *widget)
{
	...
	gx_widget_create(&scroll_widget, "greate_icon.widget", widget, 
	...
	status = gx_numeric_scroll_wheel_create(&time_wheel, "time_wheel", &scroll_widget, 1, 5, GX_STYLE_ENABLED | GX_STYLE_BORDER_NONE | GX_STYLE_TEXT_CENTER, GX_ID_NONE, &childsize);
	gx_scroll_wheel_total_rows_set(&time_wheel, 5);

	gx_widget_fill_color_set(&time_wheel, GX_COLOR_ID_CANVAS, GX_COLOR_ID_CANVAS, GX_COLOR_ID_CANVAS);
	gx_text_scroll_wheel_font_set(&time_wheel, GX_FONT_ID_SIZE_60, GX_FONT_ID_SIZE_60);
	gx_text_scroll_wheel_text_color_set(&time_wheel, GX_COLOR_ID_DISABLED_FILL, GX_COLOR_ID_HONBOW_WHITE,
										GX_COLOR_ID_HONBOW_WHITE);
	gx_scroll_wheel_row_height_set(&time_wheel, 65);

	gx_utility_rectangle_define(&childsize, 184, 163, 235, 193);
	gx_prompt_create(&min_prompt, NULL, widget, 0,
					GX_STYLE_TRANSPARENT | GX_STYLE_TEXT_LEFT | GX_STYLE_BORDER_NONE, 
					0, &childsize);
	gx_prompt_text_color_set(&min_prompt, GX_COLOR_ID_HONBOW_WHITE, 
		GX_COLOR_ID_HONBOW_WHITE, GX_COLOR_ID_HONBOW_WHITE);	
	gx_prompt_font_set(&min_prompt, GX_FONT_ID_SIZE_30);
	string.gx_string_ptr = "min";
	string.gx_string_length = strlen(string.gx_string_ptr);
	gx_prompt_text_set_ext(&min_prompt, &string);

	gx_scroll_wheel_gradient_alpha_set(&time_wheel, 200, 0);
...
void app_set_breath_time_window_init(GX_WINDOW *window)
{
	GX_RECTANGLE childsize;
	GX_BOOL result;
	gx_widget_created_test(&set_breath_time_window, &result);
	if (!result) {
		gx_utility_rectangle_define(&childsize, 0, 0, 319, 359);
		gx_widget_create(&set_breath_time_window, "set_breath_time_window", window, 
						GX_STYLE_ENABLED | GX_STYLE_TRANSPARENT, GX_ID_NONE,
						&childsize);
		gx_widget_event_process_set(&set_breath_time_window, app_set_breath_time_window_event);
		GX_WIDGET *parent = &set_breath_time_window;
		...
		set_breath_time_child_widget_create(parent);
	}
