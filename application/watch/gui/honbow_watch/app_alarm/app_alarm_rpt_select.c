...
GX_SCROLLBAR_APPEARANCE rpt_sel_list_scrollbar_properties = {
	9,						 /* scroll width                   */
	9,						 /* thumb width                    */
	0,						 /* thumb travel min               */
	0,						 /* thumb travel max               */
...
static int alarm_rpt_sel_page_info_update(ALARM_EDIT_TYPE type, ALARM_INFO_T *info);
...
static UINT alarm_rpt_sel_page_event_process(GX_WIDGET *widget, GX_EVENT *event_ptr)
{
	...
	case GX_EVENT_PEN_UP: {
		...
		if (delta_x >= 100) {
			app_alarm_page_jump(ALARM_PAGE_RPT_SEL, ALARM_OP_CANCEL);
		}
		break;
	}
	...
	case GX_SIGNAL(NEXT_OR_FINISH_BUTTON_ID, GX_EVENT_CLICKED): {
		...
		app_alarm_page_jump(ALARM_PAGE_RPT_SEL, ALARM_OP_NEXT_OR_FINISH);
		break;
	}
	default:
		break;
...
void update_radiobox_status(CUSTOM_RADIOBOX *radiobox, uint8_t rpt_list_index, ALARM_INFO_T *alarm_info)
{
	if (alarm_info != NULL) {
		if (rpt_list_index == 0) {
...
void update_alarm_info_rpt_flag(ALARM_INFO_T *alarm_info, uint8_t rpt_list_index)
{
	if (alarm_info != NULL) {
...
static UINT app_alarm_processing_function(GX_WIDGET *widget, GX_EVENT *event_ptr)
{
	switch (event_ptr->gx_event_type) {
...
static VOID rpt_sel_list_row_create(GX_VERTICAL_LIST *list, GX_WIDGET *widget, INT index)
{
	...
	if (!result) {
		...
		gx_widget_event_process_set(&row->widget, app_alarm_processing_function);
		...
		row->radio_box.color_id_select = GX_COLOR_ID_HONBOW_BLUE;
...
void app_alarm_rpt_sel_page_init(void)
{
	GX_RECTANGLE childSize;
	...
	custom_rounded_button_create(&next_finish_button, NEXT_OR_FINISH_BUTTON_ID, parent, &childSize,
								 ...
								 CUSTOM_ROUNDED_BUTTON_TEXT_LEFT);
	app_alarm_page_reg(ALARM_PAGE_RPT_SEL, &alarm_rpt_sel_page, alarm_rpt_sel_page_info_update);
}
