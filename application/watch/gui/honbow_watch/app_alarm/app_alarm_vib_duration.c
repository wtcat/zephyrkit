...
static int alarm_vib_sel_page_info_update(ALARM_EDIT_TYPE type, ALARM_INFO_T *info);
...
static UINT alarm_time_sel_page_event_process(GX_WIDGET *widget, GX_EVENT *event_ptr)
{
	...
	case GX_EVENT_PEN_UP: {
		...
		if (delta_x >= 100) {
			app_alarm_page_jump(ALARM_PAGE_VIB_DURATION, ALARM_OP_CANCEL);
		}
		break;
	}
	...
	case GX_SIGNAL(NEXT_OR_FINISH_BUTTON_ID, GX_EVENT_CLICKED): {
		...
		app_alarm_page_jump(ALARM_PAGE_VIB_DURATION, ALARM_OP_NEXT_OR_FINISH);
		break;
	}
	default:
		break;
...
void alarm_vib_duration_sel_page_init(void)
{
	...
	custom_rounded_button_create(&next_finish_button, NEXT_OR_FINISH_BUTTON_ID, parent, &childSize,
								 GX_COLOR_ID_HONBOW_BLUE, GX_PIXELMAP_ID_APP_ALARM_CHECK_BOX_SELECTED, NULL,
								 CUSTOM_ROUNDED_BUTTON_TEXT_LEFT);
	app_alarm_page_reg(ALARM_PAGE_VIB_DURATION, &alarm_vib_duration_select_page, alarm_vib_sel_page_info_update);
}
