...
#define ALARM_INFO_WIDGET_MAX_SIZE 9
typedef struct {
	ALARM_INFO_T *alarm_info;
	GX_WIDGET *widget_binding;
} ALARM_INFO_MGR_T;
static ALARM_INFO_MGR_T alarm_info_mgr[ALARM_INFO_WIDGET_MAX_SIZE];
...
CUSTOM_SLIDE_DEL_WIDGET alarm_widget_list[ALARM_INFO_WIDGET_MAX_SIZE];
Custom_RoundedButton_TypeDef alarm_add_button;
GX_PIXELMAP_BUTTON no_alarm_pixelmap;
GX_PROMPT no_alarm_tips;
...
ALARM_CHILD_T alarm_child_list[ALARM_INFO_WIDGET_MAX_SIZE];
...
static void alarm_info_mgr_sort(ALARM_INFO_MGR_T *alarm_info_mgr, uint8_t size);
...
static UINT scroll_page_event_process(GX_WIDGET *widget, GX_EVENT *event_ptr)
{
	...
	case DATA_SERVICE_EVT_ALARM_INFO_CHG: {
		GX_BOOL child_finded = GX_FALSE;
		GX_SCROLLBAR *pScroll;
		GX_VALUE top = 0;
		alarm_info_mgr_sort(alarm_info_mgr, sizeof(alarm_info_mgr) / sizeof(ALARM_INFO_MGR_T));
	...
	case GX_SIGNAL(ALARM_ADD_BUTTON_ID, GX_EVENT_CLICKED):
		...
		app_alarm_page_jump(ALARM_PAGE_MAIN, ALARM_OP_ADD_NEW);
		return status;
...
static UINT custom_slide_widget_event_process(GX_WIDGET *widget, GX_EVENT *event_ptr)
{
	...
	case GX_SIGNAL(ALARM_CHILD_BUTTON_0_ID, GX_EVENT_CLICKED): {
		...
		if ((point_record.gx_point_x > child1_rect->gx_rectangle_left) &&
			...
			for (uint8_t i = 0; i < ALARM_INFO_WIDGET_MAX_SIZE; i++) {
				if (alarm_info_mgr[i].widget_binding == (GX_WIDGET *)custom_widget) {
					app_alarm_instance_record(alarm_info_mgr[i].alarm_info);
					app_alarm_page_jump(ALARM_PAGE_MAIN, ALARM_OP_EDIT);
					break;
				}
			}
			...
			for (uint8_t i = 0; i < ALARM_INFO_WIDGET_MAX_SIZE; i++) {
				if (alarm_info_mgr[i].widget_binding == (GX_WIDGET *)custom_widget) {
					...
					if (alarm_info_mgr[i].alarm_info->alarm_flag & 0x01) {
						alarm_info_mgr[i].alarm_info->alarm_flag &= 0xfe;
						if (alarm_child->checkbox.widget.gx_widget_style & GX_STYLE_BUTTON_PUSHED) {
							custom_checkbox_basic_toggle(&alarm_child->checkbox);
						}
					} else {
						alarm_info_mgr[i].alarm_info->alarm_flag |= 0x01;
						if (!(alarm_child->checkbox.widget.gx_widget_style & GX_STYLE_BUTTON_PUSHED)) {
							custom_checkbox_basic_toggle(&alarm_child->checkbox);
						}
					}
	...
	case GX_SIGNAL(ALARM_CHILD_BUTTON_1_ID, GX_EVENT_CLICKED):
		...
		for (uint8_t i = 0; i < ALARM_INFO_WIDGET_MAX_SIZE; i++) {
			if (alarm_info_mgr[i].widget_binding == (GX_WIDGET *)custom_widget) {
				view_service_event_submit(VIEW_EVENT_TYPE_ALARM_DEL, &alarm_info_mgr[i].alarm_info, 4);
...
static UINT alarm_mian_page_event_process(GX_WIDGET *widget, GX_EVENT *event_ptr)
{
	switch (event_ptr->gx_event_type) {
	case GX_EVENT_HIDE: {
	} break;
...
void app_alarm_main_page_init(GX_WINDOW *window)
{
	GX_RECTANGLE size;
	...
	gx_widget_event_process_set(&curr_page, alarm_mian_page_event_process);
	...
	custom_rounded_button_create(&alarm_add_button, ALARM_ADD_BUTTON_ID, (GX_WIDGET *)&scrollable_alarm_page, &size,
								 GX_COLOR_ID_HONBOW_BLUE, GX_PIXELMAP_ID_APP_ALARM_ADD, NULL,
								 CUSTOM_ROUNDED_BUTTON_TEXT_LEFT);
	...
	gx_pixelmap_button_create(&no_alarm_pixelmap, NULL, &scrollable_alarm_page, GX_PIXELMAP_ID_APP_NOALARM,
							  GX_PIXELMAP_ID_APP_NOALARM, GX_PIXELMAP_ID_APP_NOALARM,
							  GX_STYLE_BORDER_NONE | GX_STYLE_ENABLED, GX_ID_NONE, &size);

	top = parent_size->gx_rectangle_top + 128;
	left = parent_size->gx_rectangle_left;
	gx_utility_rectangle_define(&size, left, top, left + 320 - 1, top + 30 - 1);
	...
	string.gx_string_ptr = "NO ALARMS";
	string.gx_string_length = strlen(string.gx_string_ptr);
	gx_prompt_text_set_ext(&no_alarm_tips, &string);

	// update display by viewservice
	view_service_alarm_info_get(&alarm_info, &alarm_info_size);
	void update_alarm_info_disp_init(ALARM_INFO_T * alarm_info, uint8_t size);
	update_alarm_info_disp_init(alarm_info, alarm_info_size);

	app_alarm_page_reg(ALARM_PAGE_MAIN, &curr_page, NULL);
}
...
static void alarm_info_mgr_sort(ALARM_INFO_MGR_T *alarm_info_mgr, uint8_t size)
{
	for (uint8_t i = 0; i < size; i++) {
		for (uint8_t j = i + 1; j < size; j++) {
			ALARM_INFO_MGR_T tmp;
			if ((alarm_info_mgr[j].alarm_info->info_avalid == ALARM_INFO_VALID_FLAG) &&
				(alarm_info_mgr[i].alarm_info->info_avalid == ALARM_INFO_VALID_FLAG)) {
			...
			} else if (alarm_info_mgr[j].alarm_info->info_avalid == ALARM_INFO_VALID_FLAG) {
				tmp = alarm_info_mgr[j];
				alarm_info_mgr[j] = alarm_info_mgr[i];
				alarm_info_mgr[i] = tmp;
			}
...
static void alarm_info_widget_update(void)
{
	...
	for (uint8_t i = 0; i < sizeof(alarm_info_mgr) / sizeof(ALARM_INFO_MGR_T); i++) {
		if ((alarm_info_mgr[i].widget_binding) &&
			(alarm_info_mgr[i].alarm_info->info_avalid == ALARM_INFO_VALID_FLAG)) {
			...
			gx_widget_resize(alarm_info_mgr[i].widget_binding, &rect_size);
			gx_widget_attach(&scrollable_alarm_page, alarm_info_mgr[i].widget_binding);

			CUSTOM_SLIDE_DEL_WIDGET *widget_custom = (CUSTOM_SLIDE_DEL_WIDGET *)alarm_info_mgr[i].widget_binding;
			// sync info to widget!
			ALARM_CHILD_T *alarm_child = CONTAINER_OF(widget_custom->child1, ALARM_CHILD_T, child1);

			if (alarm_info_mgr[i].alarm_info->repeat_flags == 0x80) {
				...
				for (uint8_t repeat_index = 0; repeat_index < 7; repeat_index++) {
					...
					if (alarm_info_mgr[i].alarm_info->repeat_flags & (1 << repeat_index)) {
			...
			init_string.gx_string_ptr = alarm_info_mgr[i].alarm_info->desc;
			init_string.gx_string_length = strlen(init_string.gx_string_ptr);
			...
			snprintf(alarm_child->time_disp_buff, sizeof(alarm_child->time_disp_buff), "%02d:%02d",
					 alarm_info_mgr[i].alarm_info->hour, alarm_info_mgr[i].alarm_info->min);
	...
	gx_widget_attach(&scrollable_alarm_page, &alarm_add_button);
	gx_widget_resize(&alarm_add_button, &rect_size);

	GX_VALUE total_height;
	GX_VALUE widget_height = 0;
	gx_widget_height_get(&scrollable_alarm_page.widget, &widget_height);
	if (alarm_info_mgr[0].alarm_info->info_avalid == ALARM_INFO_VALID_FLAG) {
		total_height = top + 65 - 1 - alarm_info_mgr[0].widget_binding->gx_widget_size.gx_rectangle_top;
	} else {
...
void update_alarm_info_disp_init(ALARM_INFO_T *alarm_info, uint8_t size)
{
	if (size != sizeof(alarm_info_mgr) / sizeof(ALARM_INFO_MGR_T)) {
		LOG_ERR("ui widget size not equal to alarm info size!");
	}

	// binding to widget fixed!
	for (uint8_t i = 0; i < sizeof(alarm_info_mgr) / sizeof(ALARM_INFO_MGR_T); i++) {
		alarm_info_mgr[i].alarm_info = &alarm_info[i];
		alarm_info_mgr[i].widget_binding = (GX_WIDGET *)&alarm_widget_list[i];
	}
