...
#include "app_stop_watch_language.h"
...
static UINT stop_watch_page_event_process(GX_WIDGET *widget, GX_EVENT *event_ptr)
{
	...
	case GX_EVENT_TIMER:
		...
		if ((timer_id == CLOCK_TIMER_ID) && (event_ptr->gx_event_target == widget)) {
			struct timespec ts;
...
static VOID record_list_callback(GX_VERTICAL_LIST *list, GX_WIDGET *widget, INT index)
{
	...
	GX_STRING init_value;
	if (g_info != NULL) {
		// LOG_INF("index = %d,counts = %d", index, g_info->counts);
...
void app_stopwatch_main_page_init(GX_WINDOW *window)
{
	...
	GX_STRING string;
	string.gx_string_ptr = app_stop_watch_get_title();
	string.gx_string_length = strlen(string.gx_string_ptr);
	...
	custom_rounded_button_create(&start_and_pause_button, BUTTON_START_PAUSE_ID, &curr_widget, &childSize,
								 GX_COLOR_ID_HONBOW_DARK_GRAY, GX_PIXELMAP_ID_WIDGETS_START_ICON, NULL,
	...
	gx_utility_rectangle_define(&childSize, 50, 151, 320 - 50 - 1, 276);
