...
extern void alarm_table_refresh(GX_VERTICAL_LIST *list);
/*************************************************************************************/
...
#if 0 // TODO: need improve!
	case GX_SIGNAL(ID_ADD, GX_EVENT_CLICKED):
		alarm_add_del_test();
		// alarm_table_refresh(); //TODO
		break;
#endif
	}
...
void alarm_add_del_test(void)
{
	...
	alarm_add.alarm_am_pm = GX_STRING_ID_AM;
	char time[3];
...
void alarm_table_refresh(GX_VERTICAL_LIST *list)
{
	UCHAR total_cnt, max_cnt;

	alarm_list_ctl_get_total(&total_cnt, &max_cnt);

	if (list->gx_vertical_list_total_rows != total_cnt) {
		...
		for (uint8_t i = 0; i < cnt_child; i++) {
			gx_widget_attach((GX_WIDGET *)list,
							 &alarm_list_row_memory[i].widget);
		}

		custom_vertical_list_total_rows_set(list, total_cnt);
	}
}
...
UINT alarm_screen_event_handler(GX_WINDOW *window, GX_EVENT *event_ptr)
{
	...
	case GX_EVENT_SHOW:
		alarm_list_ctl_init();
		// alarm_table_refresh(); //TODO: need edit
		break;
	default:
		break;
	}
