MEMORY {
    PSRAM (rw) : ORIGIN = 1442840576, LENGTH = 2097152
}
 OUTPUT_FORMAT("elf32-littlearm")
_region_min_align = 32;
MEMORY
    {
    FLASH (rx) : ORIGIN = (0x0 + 0xc000), LENGTH = (2048*1K - 0xc000)
    SRAM (wx) : ORIGIN = 0x10010000, LENGTH = (704 * 1K)
    ...
    IDT_LIST (wx) : ORIGIN = (0x10010000 + (704 * 1K)), LENGTH = 2K
    }
ENTRY("__start")
SECTIONS
    {
 ...
 *(.iplt)
 ...
 _image_rom_start = (0x0 + 0xc000);
    rom_start :
 {
. = 0x0;
. = ALIGN(4);
. = ALIGN( 1 << LOG2CEIL(4 * 32) );
. = ALIGN( 1 << LOG2CEIL(4 * (16 + 34)) );
_vector_start = .;
 ...
 *(.igot)
 ...
 _image_ram_start = .;
.stp_node : ALIGN_WITH_INPUT SUBALIGN(4)
{
  __stp_node_start = .;
  KEEP(*(SORT(.stp_node_*)));
  __stp_node_end = .;
} > FLASH
.app_node : ALIGN_WITH_INPUT SUBALIGN(4)
{
  __app_node_start = .;
  KEEP(*(SORT(.app_node_*)));
  __app_node_end = .;
 ...
 *(".kernel_noinit.*")
    ...
    _image_ram_end = .;
    _end = .;
    __kernel_ram_end = 0x10010000 + (704 * 1K);
 ...
 .psram_noinit (NOLOAD) : SUBALIGN(4)
 {
